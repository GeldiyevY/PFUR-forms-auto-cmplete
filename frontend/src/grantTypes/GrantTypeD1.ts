...
// Мин. число публикаций руководителя WoS/Scopus, п.3.3.1 КД D.1.
...
export class GrantTypeD1 extends GrantType {
  ...
  // Область наук управляет порогом КПЭ-2 и лимитами сметы (п.6.3.4 / 6.4 КД D.1).
  directions = [
    new DirectionNaturalScience({
      onApply: (reg) => {
        reg.get<KpiElement>("kpi_table")?.copyWith({
          // КПЭ-2 «Привлечение внешнего финансирования» — не менее 1000 тыс. руб. ежегодно
          kpiThresholds: [{ field: "kpe2", stage: "all", min: 1000 }],
        });
        reg.get<BudgetElement>("budget_table")?.copyWith({
          budgetThresholds: [
            // НТУ ≤ 10% (п.6.3.3), Open Access не учитывается
    ...
    new DirectionSocialScience({
      onApply: (reg) => {
        reg.get<KpiElement>("kpi_table")?.copyWith({
          // КПЭ-2 — не менее 500 тыс. руб. ежегодно
          kpiThresholds: [{ field: "kpe2", stage: "all", min: 500 }],
        });
        reg.get<BudgetElement>("budget_table")?.copyWith({
          budgetThresholds: [
            { line: "services", year: "all", maxPercent: 0.1 },
  ...
  form2 = [
    ...
    new TextAreaField({
      ...
      hint: "Место работы, должность; образование, ученая степень/звание; статус и планируемая должность в коллективе; профессиональный уровень (публикации, патенты, опыт руководства НИР/НИОКР).",
      required: true,
      testValue:
  ...
  form3 = [
    new KpiElement({
      grantType: "D1", // строки 1-6, 8, 9, 10 (без строки 7 R1)
      firstFieldCriteria: {
        "Top 1%": 40,
        "Top 5%": 30,
        "Top 10%": 20,
      ...
      // БРС минимум = КАТЕГОРИЯ × НАПРАВЛЕНИЕ. 1-й год: 20% (А) / 35% (Б)
      // от минимальной константы БРС (КПЭ-2 засчитывается в эту сумму).
      minPoints: {
        А: [
          {
            direction: "Фундаментальное",
            minPerStage: [{ stage: 1, min: 66 }],
            minTotal: 330,
          },
          {
            direction: "Прикладное",
            minPerStage: [{ stage: 1, min: 60 }],
            minTotal: 300,
          },
        ],
        Б: [
          {
            direction: "Фундаментальное",
            minPerStage: [{ stage: 1, min: 63 }],
            minTotal: 180,
          },
          {
            direction: "Прикладное",
            minPerStage: [{ stage: 1, min: 56 }],
            minTotal: 160,
          },
        ],
      },
  ...
  form4 = [
    new BudgetElement({
      ...
      grantType: "D1", // добавляет поле left_over_explanation
  ...
  application = [
    ...
    new DateField({
      ...
      verificationFunction: (value, reg) => {
        ...
        if (age > 39) {
          return `Возраст руководителя на ${cutoff} не должен превышать 39 лет; сейчас ${age}`;
        }
        return null;
      },
    }),
    ...
    new TextField({
      ...
      detail: "Рекомендуем посмотреть п.3.3.4",
      emptyValue: "",
    ...
    new NumberField({
      ...
      detail: "Рекомендуем проверить п. 3.3.1",
      emptyValue: "",
      testValue: "45",
      verificationFunction: (value, reg) =>
    ...
    new Row({
      id: "projects",
      label:
        "Участие в научных проектах (грантах), хоздоговорных НИР, НИР по гос. заданию (за период с 01.01.2022)",
      increasable: true,
      minRows: 1,
      elements: [
        new RowElementTextField({
