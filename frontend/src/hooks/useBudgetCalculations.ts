...
export function useBudgetCalculations(isCategoryA: boolean) {
  ...
  const totals = BUDGET_KEYS.map((key) => {
    ...
    const total = line.year1 + line.year2 + (isCategoryA ? line.year3 : 0);
    return { ...line, total };
  });

  const year1Total = totals.reduce((s, l) => s + l.year1, 0);
  const year2Total = totals.reduce((s, l) => s + l.year2, 0);
  const year3Total = isCategoryA ? totals.reduce((s, l) => s + l.year3, 0) : 0;
